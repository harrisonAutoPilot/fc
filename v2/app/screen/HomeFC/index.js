...
const Home = ({ navigation }) => {
  ...
  const CategoryToggle = useCallback(({ onPress,category }) => (
  ...
  const SoundToggle = useCallback(({ onPress, onPressNew, playSoundId }) => (
            ...
            isInCenter ?
              <View key={item.id} style={styles.videoCardt}>
                ...
                <TouchableOpacity style={styles.titleWrapper}>
                  ...
                  <Image source={require("@Assets2/image/verified.png")} style={styles.verifyImg} />
                  </View>
                ...
                {item.id === selected && onEnd ?
                  ...
                  {detailsValue == item.id && showPreview ?
              null
              :
  ...
  return (
   ...
   <OffsetYProvider
      ...
      <CategoryDetails
        ...
        title={catItem}
        message="Contents that propels individuals to engage in positive goal-directed behavior."
      />
