...
const MessageBottomSheet = (props) => {
  ...
  const OptionHelper = useCallback(
    ({ onPress, item }) => (
      <TouchableOpacity
        ...
        onPress={() => {
          setDisplayOption(false), setNewValue(item);
        }}
      >
        <View>
      ...
      <View style={styles.messageCard}>
        ...
        <View style={styles.userCommentCover}>
          ...
          <View style={styles.userCommentDate}>
            ...
            <Text style={styles.commenterDate}>
              {moment(item?.pivot?.created_at).utcOffset("-05:00").fromNow()}
            </Text>
          </View>
          <View>
  ...
  return (
    <BottomSheet
      ...
      hasDraggableIcon
      ref={props?.bottomSheetRefMessage}
      height={480}
    >
      ...
      <View style={styles.bottomContainer}>
        <View style={styles.scrollContainer}>
          <ScrollView horizontal={true} showsHorizontalScrollIndicator={false}>
            {displayOption
              ? data?.map((data) => {
                  return <OptionHelper item={data.name} />;
                })
              : null}
          </ScrollView>
        </View>
          ...
          <View style={styles.innerFlex}>
            {userImg && (
              <Image
                ...
                source={{
                  uri: `${Config.IMG_URL}${props?.userData?.avatar?.url}`,
                }}
                resizeMode="cover"
              />
            )}
